...
function move(dx, dy) { if (!running || won) return; const nx = player.x + dx, ny = player.y + dy; if (!map[ny]?.[nx]) { player.x = nx; player.y = ny; player.bob = .35; const found = crystals.findIndex(c => c.x === nx && c.y === ny); if (found >= 0) { crystals.splice(found, 1); crystalCount.textContent = +crystalCount.textContent + 1; } if (nx === exit.x && ny === exit.y && !crystals.length) victory(); monsterTurn(); } }
...
function hurt() { health--; healthDisplay.textContent = '♥'.repeat(Math.max(0,health)) + '♡'.repeat(Math.max(0,3-health)); if (health <= 0) gameOver(); }
function lanternBurst() { if (!running || won) return; burst = 1; const before=monsters.length; monsters = monsters.filter(m => Math.abs(m.x-player.x)+Math.abs(m.y-player.y)>4); if (before === monsters.length) monsterTurn(); }
...
function gameOver() { won = true; overlay.innerHTML = '<p class="eyebrow">LANTERN EXTINGUISHED</p><h2>The dark takes you.</h2><p>Something unseen carries on through the tunnels.</p><button id="again">Try another descent</button>'; overlay.classList.remove('hidden'); document.querySelector('#again').onclick = begin; }
function draw() {
...
window.addEventListener('keydown', e => { const k=e.key.toLowerCase(), dirs={arrowup:[0,-1],w:[0,-1],arrowdown:[0,1],s:[0,1],arrowleft:[-1,0],a:[-1,0],arrowright:[1,0],d:[1,0]}; if (dirs[k]) { e.preventDefault(); if (!e.repeat) move(...dirs[k]); } if (k === ' ') { e.preventDefault(); if (!e.repeat) lanternBurst(); } });
